...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://blog.zyg.tw/soga/latest.json" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/soga.tar.gz https://blog.zyg.tw/soga/core/soga-cracked-linux64.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://blog.zyg.tw/soga/core/soga-cracked-linux64.tar.gz"
    ...
    curl -o /usr/bin/soga -Ls https://blog.zyg.tw/soga/install/soga.sh
