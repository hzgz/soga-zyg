...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/hzgz/soga-zyg/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/soga.tar.gz https://github.com/hzgz/soga-zyg/releases/download/2.0.6/soga-cracked-linux64.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/hzgz/soga-zyg/releases/download/2.0.6/soga-cracked-linux64.tar.gz"
    ...
    if [[ ! -f /etc/soga/soga.conf ]]; then
        ...
        echo -e ""
        echo -e "RManLuo 大佬破解 HEDAY修改并制作"
    else
    ...
    curl -o /usr/bin/soga -Ls https://raw.githubusercontent.com/hzgz/soga-zyg/master/soga.sh
